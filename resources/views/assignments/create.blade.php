<x-app-layout>
    <x-slot name="title">Tugas Baru</x-slot>

    <x-ui.page-header title="Buat Tugas" :subtitle="$subject->name"
        :back-href="route('class-rooms.subjects.assignments.index', [$classRoom, $subject])" back-label="Tugas" />

    <div class="mx-auto max-w-xl">
        <form method="POST" action="{{ route('class-rooms.subjects.assignments.store', [$classRoom, $subject]) }}" class="card p-6">
            @csrf

            <div>
                <label for="title" class="label">Judul</label>
                <input id="title" name="title" type="text" class="input" value="{{ old('title') }}" required autofocus placeholder="mis. Proyek Akhir" />
            ...
            <div class="mt-4">
                ...
                <input id="due_date" name="due_date" type="datetime-local" class="input" value="{{ old('due_date') }}" />
            ...
            <div class="mt-4">
                ...
                <div class="grid grid-cols-2 gap-3">
                    @php($mode = old('submission_mode', 'none'))
                    <label class="cursor-pointer rounded-md border p-4 transition focus-within:ring-2 focus-within:ring-rose-600 focus-within:ring-offset-2 has-[:checked]:border-rose-700 has-[:checked]:bg-rose-50 has-[:checked]:ring-1 has-[:checked]:ring-rose-600">
                ...
                <div id="submission-options" class="mt-4 rounded-md border border-gray-200 bg-gray-50 p-4">
                    <div>
                        ...
                        <input id="max_files" name="max_files" type="number" min="1" max="20" class="input" value="{{ old('max_files', 1) }}" />
                    ...
                    <div class="mt-4">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @php($exts = (array) old('allowed_extensions', []))
                    ...
                    <div class="mt-4">
                        ...
                        <input id="max_file_size_kb" name="max_file_size_kb" type="number" min="100" max="102400" class="input" value="{{ old('max_file_size_kb', 10240) }}" />
            ...
            <div class="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:items-center sm:justify-between">
                <x-ui.button href="{{ route('class-rooms.subjects.assignments.index', [$classRoom, $subject]) }}" type="outline" class="w-full sm:w-auto">Batal</x-ui.button>
                <x-ui.button :submit="true" class="w-full sm:w-auto">Buat Tugas</x-ui.button>
            </div>
        </form>
    </div>
