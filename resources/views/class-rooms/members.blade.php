...
<x-ui.page-header title="Kelola Anggota" :subtitle="$classRoom->name"
        :back-href="route('class-rooms.show', $classRoom)" :back-label="$classRoom->name">
</x-ui.page-header>

    <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
        {{-- Current members --}}
<x-ui.card :title="'Anggota Saat Ini ('.$members->total().')'">
    <x-slot name="action">
            ...
            <div class="space-y-2">
                @forelse ($members as $member)
                    <div class="flex items-center justify-between gap-3 rounded-md p-2.5 transition hover:bg-gray-50">
                        ...
                        <form method="POST" action="{{ route('class-rooms.members.destroy', [$classRoom, $member]) }}" data-confirm="Hapus {{ $member->name }} dari kelas ini?" onsubmit="return confirm(this.dataset.confirm)">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                    <x-ui.empty-state title="Belum ada anggota" description="Tambahkan mahasiswa menggunakan formulir." />
                @endforelse
            </div>
        ...
        {{-- Add members --}}
        <x-ui.card title="Tambah Anggota">
            <form method="POST" action="{{ route('class-rooms.members.store', $classRoom) }}">
                @csrf
                <label for="user_ids" class="label">Mahasiswa</label>
